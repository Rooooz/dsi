...
token = "OTYyNTUyNjQ4ODYxNjIyMzQ0.YlJNAg.U6mpPJQ2MQo1e0Oxd-he4fGH7wQ"
...
async def sms(ctx, phone, amount:int):
    ...
    if (amount < 151):
      ...
      embes = discord.Embed(title="SmS", description="Bot by : hack 40",color=0xff4612)
      ...
      BBot(phone,amount)
    	...
    	await ctx.channel.send("อย่ายิงเกิน150นะ🤕")
...
@bot.command()
async def help(ctx):
	emBed = discord.Embed(title="Bot BY: hack 40",description="วิธิใช้",color=0xff4612)
	emBed.add_field(name="🔥😈",value="!sms [เบอร์] [จำนวน1-150]")
